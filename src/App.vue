...
import { Link, UserFilled, Star, Message, Briefcase, Trophy, CopyDocument, Moon, Sunny } from '@element-plus/icons-vue'
...
import { projects, techEdges, techStack, type ProjectMeta, type TechLevel } from './data/techStack'
...
const getVisibleHighlights = (project: ProjectMeta) => {
  return expandedProjects.value.has(project.title) ? project.highlights : project.highlights.slice(0, 2)
}
...
onMounted(() => {
  ...
  const initAmbient = (container: HTMLElement | null) => {
    if (!container) return
    const dots = Array.from(container.querySelectorAll<HTMLElement>('.ambient-dot'))
  ...
  const randomSignedSpeed = () => (Math.random() > 0.5 ? 1 : -1) * randomSpeed()

  const reorientAfterCollision = (dotState: AmbientDotState['dotStates'][number], axis: 'x' | 'y', edge: 'min' | 'max') => {
    if (axis === 'x') {
  ...
  ambientStates.forEach((state) => state.dots.forEach((dot) => randomizeDotColor(dot)))
...
<template>
  <div class="page-shell">
    <div ref="ambientLeftRef" class="side-ambient side-ambient-left" aria-hidden="true">
    ...
    <div ref="ambientRightRef" class="side-ambient side-ambient-right" aria-hidden="true">
      <span class="ambient-marquee"></span>
      <span class="ambient-marquee"></span>
      <span class="ambient-dot"></span>
      <span class="ambient-dot"></span>
      ...
      <section class="grid">
        <el-card class="panel stack-panel" shadow="hover">
          ...
          <div class="tags-group-list">
            <section v-for="group in techLevelGroups" :key="group.key" class="tags-group">
              <h4 class="tags-group-title">{{ group.label }}</h4>
              <div class="tags">
                <el-tag
                  v-for="item in techStack.filter((tech) => tech.level === group.key)"
                  :key="item.name"
                  round
                  class="tech-tag"
                  :class="{
            ...
            <li v-for="project in projects" :key="project.title">
              ...
              >
                ...
                <ul class="project-highlights">
                  <li v-for="item in getVisibleHighlights(project)" :key="item">{{ item }}</li>
                </ul>
                <button
                ...
                >
                  {{ expandedProjects.has(project.title) ? '收起亮点' : '展开亮点' }}
                </button>
